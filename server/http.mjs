...
export async function handleApi(request,service){
 ...
 if(path.join('/')==='auth/login'&&method==='POST'){const input=await jsonBody(request);fields(input,['identifier','email','username','dpi','password','tenant_id']);const result=await service.auth.login(input);return response(result.user,200,{'set-cookie':cookie(result.token,secure),'x-request-id':requestId})}
