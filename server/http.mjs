...
export async function handleApi(request,service){
 ...
 if(path.join('/')==='auth/municipalities'&&method==='GET')return response(await service.auth.municipalities())
 if(path.join('/')==='auth/register'&&method==='POST'){const input=await jsonBody(request);fields(input,['name','email','dpi','password','tenant_id']);return response(await service.auth.register(input),201)}
 if(path.join('/')==='auth/google/config'&&method==='GET')return response(await service.googleAuthConfig())
 ...
 if(path.join('/')==='auth/login'&&method==='POST'){const input=await jsonBody(request);fields(input,['identifier','email','username','dpi','password','tenant_id','administrative_login']);const result=await service.auth.login(input);return response(result.user,200,{'set-cookie':cookie(result.token,secure),'x-request-id':requestId})}
 ...
 if(path[0]==='me'&&path[1]==='onboarding'&&path.length===2){if(method==='GET')return response(await service.userOnboarding(session));if(method==='POST')return response(await service.updateUserOnboarding(session,await jsonBody(request)))}
 ...
 if(path.join('/')==='admin/google-auth'&&method==='POST')return response(await service.updateGoogleAuth(session,await jsonBody(request)))
 if(path[0]==='tenants'){
   ...
   if(path[2]==='team'){
     ...
     if(path[3]==='registrations'&&path.length===5&&path[4]&&method==='POST'){const input=await jsonBody(request);fields(input,['action','responsibility']);if(input.action==='approve')return response(await service.approveRegistration(session,id,path[4],{responsibility:input.responsibility}));invariant(input.action==='reject','INVALID_INPUT','Unsupported registration action');return response(await service.rejectRegistration(session,id,path[4]))}
   ...
   if(path[2]==='creator-insights'){if(path.length===3){if(method==='GET')return response(await service.creatorInsights(session,id));if(method==='POST')return response(await service.createCreatorInsight(session,id,await jsonBody(request)),201)}if(path.length===4&&path[3]==='discovery'&&method==='GET')return response(await service.creatorInsightDiscovery(session,id));if(path.length===4&&method==='GET')return response(await service.creatorInsight(session,id,path[3]));if(path.length===5&&method==='POST'){if(path[4]==='plan')return response(await service.updateCreatorInsightPlan(session,id,path[3],await jsonBody(request)));if(path[4]==='approve')return response(await service.approveCreatorInsight(session,id,path[3],await jsonBody(request)))}}
 ...
 if(path[0]==='jobs'){
   ...
   if(path[2]==='audio-assets'&&path.length===3&&method==='POST'){const bytes=await bodyBytes(request,40*1024*1024);return response(await service.uploadAudioAsset(session,id,{bytes,mime_type:request.headers.get('content-type')?.split(';')[0]}),201)}
   if(path[2]==='audio-finishing'&&path.length===3&&method==='POST')return response(await service.configureAudioFinishing(session,id,await jsonBody(request)),202)
