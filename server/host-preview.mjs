...
async function boundedAuthPost(request){invariant(request.headers.get('content-type')?.split(';')[0]==='application/json','UNSUPPORTED_MEDIA_TYPE','Send JSON input',415);const reader=request.body?.getReader();let size=0;const chunks=[];if(reader)try{while(true){const {value,done}=await reader.read();if(done)break;size+=value.byteLength;invariant(size<=2048,'BODY_TOO_LARGE','Authentication input is too large',413);chunks.push(value)}}catch(e){await reader.cancel().catch(()=>{});throw e}let body;try{body=JSON.parse(Buffer.concat(chunks).toString())}catch{throw new ProductError('INVALID_JSON','Send valid authentication input')};if(body.action==='register'){fields(body,['action','name','email','dpi','password','tenant_id']);return{action:'register',body:Object.fromEntries(['name','email','dpi','password','tenant_id'].filter(k=>body[k]!==undefined).map(k=>[k,body[k]]))}}fields(body,['action','identifier','email','username','dpi','password','tenant_id','administrative_login','locale','next']);invariant(body.action===undefined||body.action==='login','INVALID_INPUT','Unsupported authentication action');invariant(body.locale===undefined||['en','es'].includes(body.locale),'INVALID_INPUT','Unsupported locale');invariant(body.next===undefined||['/es','/en','/dashboard','/'].includes(body.next),'INVALID_INPUT','Unsupported entry path');return{action:'login',body:Object.fromEntries(['identifier','email','username','dpi','password','tenant_id','administrative_login'].filter(k=>body[k]!==undefined).map(k=>[k,body[k]]))}}
