...
async function boundedLogin(request){invariant(request.headers.get('content-type')?.split(';')[0]==='application/json','UNSUPPORTED_MEDIA_TYPE','Send JSON input',415);const reader=request.body?.getReader();let size=0;const chunks=[];if(reader)try{while(true){const {value,done}=await reader.read();if(done)break;size+=value.byteLength;invariant(size<=2048,'BODY_TOO_LARGE','Login input is too large',413);chunks.push(value)}}catch(e){await reader.cancel().catch(()=>{});throw e}let body;try{body=JSON.parse(Buffer.concat(chunks).toString())}catch{throw new ProductError('INVALID_JSON','Send valid login input')};fields(body,['email','username','password','locale','next']);invariant(body.locale===undefined||['en','es'].includes(body.locale),'INVALID_INPUT','Unsupported locale');invariant(body.next===undefined||['/es','/en','/dashboard','/'].includes(body.next),'INVALID_INPUT','Unsupported entry path');return Object.fromEntries(['email','username','password'].filter(k=>body[k]!==undefined).map(k=>[k,body[k]]))}
