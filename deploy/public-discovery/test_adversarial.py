...
from materialize import fixture_modules, reconstruct
host, ctl, original, oldctl = fixture_modules()
BASE={'access_policy':'form_session_single_operator','hostname':'media-factory.example.org','edge_alias':'media-factory-frontend','edge_port':3000,'blocked_paths':['/.env*']}
ALLOWED=['/robots.txt','/sitemap.xml','/llms.txt','/llm.txt','/opengraph-image']
CANON = pathlib.Path(__import__('os').environ.get('PUBLIC_DISCOVERY_CANONICAL_ROOT','/shared-auth/deployment/host-reconciler'))
ORIGINALS = {'host_reconciler.py':'074cfea8afb469c112c87724e892862ffd8dfb2e52863d160349eb8509ee69bd', 'productctl.py':'f282304749348053bec91fc9607dd1b06f844cb0c9c1c2fc05e475ab5cb8ab69'}
 ...
 def test_canonical_original_hashes_and_exact_zero_fuzz_patch(self):
  ...
  with tempfile.TemporaryDirectory(prefix='discovery-patch-') as tmp:
   ...
   for name in ['host_reconciler','productctl']:
    shutil.copyfile(CANON/f'{name}.py',dest/f'host-reconciler/{name}.py')
    ...
    self.assertEqual((dest/f'host-reconciler/{name}.py').read_bytes(),reconstruct(CANON,P)[f'{name}.py'][1])
  for path,expected in ORIGINALS.items(): self.assertEqual(digest(CANON/path),expected,path)
