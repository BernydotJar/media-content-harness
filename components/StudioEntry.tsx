...
export default function StudioEntry() {
 ...
 const [busy,setBusy]=useState(false),[error,setError]=useState<ApiError|null>(null),[googleAvailable,setGoogleAvailable]=useState(false),[googleError,setGoogleError]=useState('');
 ...
 return <main className={styles.entry}>
  ...
  <section id="studio-access" className={styles.access} aria-labelledby="access-title">
   <div className={styles.accessBody}><div className={styles.number} aria-hidden="true">02 / ENTRA AL ESTUDIO</div><p className={styles.eyebrow}>DE LA IDEA A LA ENTREGA</p><h2 id="access-title">Vamos a crear.</h2><p className={styles.accessDescription}>Entra a tus espacios de trabajo.<br/>Tu próxima historia te espera.</p>{googleAvailable&&<><a className={styles.google} href="/api/v1/auth/google/start"><span className={styles.googleMark} aria-hidden="true">G</span><span>Continuar con Google</span><Icon name="arrow"/></a><p className={styles.googleHint}>Usa el correo al que te invitaron. Google confirma tu identidad; Media Factory decide a qué marca puedes entrar.</p><div className={styles.divider}><span>o acceso administrativo</span></div></>}{googleError&&<div className={styles.googleError} role="alert">{googleError}</div>}<form onSubmit={submit} aria-busy={busy}><label className={styles.field}><span>Usuario o correo</span><input name="email" type="text" autoComplete="username" placeholder="Tu usuario" required/></label><label className={styles.field}><span>Contraseña</span><input name="password" type="password" autoComplete="current-password" required/></label><Problem error={error}/><button className={styles.enter} disabled={busy}>{busy?'Entrando…':'Entrar al estudio'}<Icon name="arrow"/></button></form><p className={styles.help}>{googleAvailable?'El acceso con contraseña queda disponible para el propietario o como contingencia.':'Tu organización gestiona el acceso. Si necesitas una cuenta, contacta a la persona responsable de tu espacio.'}</p></div>
