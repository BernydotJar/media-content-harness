...
export function QuickCreate({ tenants, loading = false }: { tenants: Workspace[]; loading?: boolean }) {
  ...
  return <DropdownMenu.Root>
    <DropdownMenu.Trigger className={styles.trigger} disabled={loading} aria-busy={loading || undefined}>
      <span aria-hidden="true" className={styles.plus}>+</span>Crear<Chevron down />
    </DropdownMenu.Trigger>
    <DropdownMenu.Portal>
      <DropdownMenu.Content className={styles.content} sideOffset={8} align="end" collisionPadding={12} loop aria-label="Crear contenido">
        ...
        {tenants.length ? modes.map(mode => <DropdownMenu.Sub key={mode.path}>
          ...
          <DropdownMenu.Portal>
            <DropdownMenu.SubContent className={styles.content} sideOffset={6} alignOffset={-6} collisionPadding={12} loop aria-label={'Espacios para ' + mode.label.toLowerCase()}>
