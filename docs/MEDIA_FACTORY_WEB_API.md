...
| `/api/v1/tenants/:id/creator-insights/discovery` | GET | Workspace-only discovery projection: chronological signals, content-gap counts, draft/approved counts and signal-type totals; never implies live platform analytics |
...
Creator Content Insight uses a separate tenant-bound signal → topic → content-gap → plan → human-review → ready-to-create graph. Its draft contract contains `idea`, `title`, `description` and `hashtags`, is explicitly general-audience/informational, and never auto-publishes. User-provided URLs remain reference metadata and `live_platform_data` stays false until an authorized platform connector actually supplies live data. Public-affairs workspaces reject direct electoral persuasion and calls to vote in this planning surface in addition to the existing sensitive-trait and voter-microtargeting safeguards. The V22 discovery projection adds workspace-only counts and filtering over those stored signals. It is explicitly chronological (`created_at_desc`) rather than a recommendation ranking, and reports `source_scope = workspace-recorded-signals` plus `live_platform_data = false` so internal observations cannot be confused with TikTok or other platform demand metrics.
