...
if(states['WEB005-DEPLOYMENT']!=='running'||states['WEB006-RELEASE']!=='spec_ready')throw Error('Unexpected graph state; inspect before retrying');
...
await gate('WEB005-DEPLOYMENT',[
 ['critique','deployment_recovery',dir+'deployment-critic.md','Read-only final deployment Critic of installed controller, signed release, host and public proofs'],
 ['critique','graph_engineer_recovery','progress/media-factory-web/WEB005-discovery-controller-critic.md','Previously performed independent controller source Critic and adversarial verification; reused unchanged source'],
 ['independent_verification','baseline_audit',dir+'public-verification.json','python3 /tmp/media-factory-public-verifier.py c648711cd2cf53d6371c1de5504165f4b423eda8'],
 ['independent_verification','existing-host-deployment-reconciler',dir+'host-reconcile.json','Existing host reconciler request 3f932cee-be65-4206-8460-3e442b08c355: immutable release, readiness, recovery, HTTPS and session gates']
]);
await graph.recordApproval({...paths,node:'WEB006-RELEASE',actor:'user-request-2026-09-12-completion',scopeHash:await hash('specs/media-factory-web/requirements.md'),note:'Actual user authorizes software completion, commit and publication through existing mechanisms, with no repeated completed tests. Release gate closes delivered controlled software and documents PARTIAL_WITH_DOCUMENTED_BLOCKERS; it does not approve customer concepts, media, paid generation or social publication.'});
await move('WEB006-RELEASE','approved','Existing user software release authorization recorded after dependency completion');
await move('WEB006-RELEASE','ready','Deployment dependency is done with actual HTTPS and independent verification');
const ready=await graph.readyNodes(paths);if(!ready.some(n=>(typeof n==='string'?n:n.id)==='WEB006-RELEASE'))throw Error('Release node not returned by actual READY scheduler');
