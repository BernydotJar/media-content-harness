...
export async function verifyQuickCreateEmpty(page) {
  ...
  await create.click()
  await expect(page.getByRole('menuitem',{name:/Agregar una marca/})).toHaveAttribute('href','/workspaces')
  await expect(page.getByRole('menuitem',{name:/Planear una semana/})).toHaveCount(0)
...
export async function verifyCreativeNavigation({page,context,origin,temp}) {
  ...
  const create=page.getByRole('button',{name:'Crear',exact:true})
  const weekly=page.getByRole('menuitem',{name:/^Planear una semana/})
  const free=page.getByRole('menuitem',{name:/^Crear con mis palabras/})
  ...
  checks.push('quick create keyboard selection navigates to the existing tenant simple creation flow')
  ...
  try {
    ...
    await touchPage.getByRole('menuitem',{name:/^Planear una semana/}).tap()
