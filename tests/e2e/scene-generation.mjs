...
export async function runSceneGenerationE2E({origin,accounts,temp,firstImage,acceptedImage,environmentImage,finalVideo}){
 ...
 const login=async account=>{const body={email:account.email,password:account.password,...(account.id==='scene-owner'?{administrative_login:true}:{tenant_id:'firmes-scene'})};const response=await context.request.post(origin+'/api/v1/auth/login',{headers:{Origin:origin},data:body});assert.equal(response.status(),200,await response.text());await page.goto(origin+'/dashboard');await settleAuthenticatedEntry(page)}
 ...
 try{
  ...
  await expect(page.getByRole('heading',{name:'Dale vida al personaje'})).toBeVisible()
  await expect(page.getByRole('button',{name:/Crear video con personaje/})).toBeVisible()
  await expect(page.getByRole('button',{name:/Agregar como sticker/})).toBeVisible()
  await expect(page.getByText('Requiere Gemini API',{exact:true})).toBeVisible()
  await expect(page.getByRole('button',{name:'Conecta Gemini para generar'})).toBeDisabled()
  await page.getByRole('button',{name:/Agregar como sticker/}).click();await expect(page.getByRole('heading',{name:'Usa el Caballito sobre un video que ya tienes.'})).toBeVisible();await expect(page.getByText(/no genera al personaje dentro de la escena/i)).toBeVisible();checks.push('V20 separa explícitamente video con personaje y sticker; sin credencial no finge generación automática.')
  await page.getByRole('button',{name:/Crear video con personaje/}).click();await expect(page.getByRole('heading',{name:'Dale vida al personaje'})).toBeVisible();await page.getByText('Herramienta de creación · opcional',{exact:true}).click();await page.getByRole('button',{name:/Generación externa/}).click();await expect(page.getByText('Generación externa seleccionada.')).toBeVisible()
  ...
  await expect(page.getByRole('heading',{name:'Look y movimiento'})).toBeVisible()
  await expect(page.getByText('FIRMES siempre visible')).toBeVisible()
  ...
  await expect(page.getByLabel('Look del avatar')).toHaveValue('avatar_outfit_firmes_beach_v1')
  await expect(page.getByLabel('Movimiento del avatar')).toHaveValue('avatar_motion_walk_v1')
  await expect(page.getByRole('textbox',{name:'Lugar',exact:true})).toHaveValue('A bright tropical beach')
  await page.getByRole('button',{name:'Ver instrucción'}).click()
  await page.getByText('Detalles de la instrucción preparada',{exact:true}).click()
  const beachPrompt=page.locator('pre').filter({hasText:'AVATAR IDENTITY LOCK'})
  await expect(beachPrompt).toContainText('AVATAR OUTFIT')
  await expect(beachPrompt).toContainText('Playa')
  await expect(beachPrompt).toContainText('Lentes de sol')
  await expect(beachPrompt).toContainText('Chanclas')
  await expect(beachPrompt).toContainText('FIRMES BRAND MARKER POLICY')
  await expect(beachPrompt).toContainText('AVATAR MOTION PROFILE')
  checks.push('Avatar reutilizable compila playa + lentes + chanclas + caminar sin perder identidad ni distintivo FIRMES.')
  await page.getByText('Detalles de la instrucción preparada',{exact:true}).click()

  await page.getByRole('button',{name:'Espacio',exact:true}).click()
  await expect(page.getByLabel('Look del avatar')).toHaveValue('avatar_outfit_firmes_space_v1')
  await expect(page.getByRole('textbox',{name:'Lugar',exact:true})).toHaveValue('A cinematic orbital space environment')
  await page.getByRole('button',{name:'Ver instrucción'}).click()
  await page.getByText('Detalles de la instrucción preparada',{exact:true}).click()
  const spacePrompt=page.locator('pre').filter({hasText:'AVATAR IDENTITY LOCK'})
  await expect(spacePrompt).toContainText('white astronaut suit')
  await expect(spacePrompt).toContainText('Casco espacial')
  await expect(spacePrompt).toContainText('visible FIRMES')
  checks.push('El mismo avatar compila traje espacial + casco espacial manteniendo identidad y marca.')
  await page.getByText('Detalles de la instrucción preparada',{exact:true}).click()

  await page.getByRole('button',{name:'Antigua',exact:true}).click()
  await expect(page.getByRole('textbox',{name:'Lugar',exact:true})).toHaveValue('Calle del Arco, Antigua Guatemala, Guatemala')
  const environmentResponse=page.waitForResponse(response=>response.url().includes('/scene-references/ENVIRONMENT_ONLY')&&response.request().method()==='POST');await page.locator('input[type=file][accept=\"image/png,image/jpeg\"]').first().setInputFiles(environmentImage);const environmentUpload=await environmentResponse;assert.ok(environmentUpload.ok(),'environment upload failed: '+environmentUpload.status());await expect(page.getByText('Referencia del lugar cargada. Solo se usará para el entorno.')).toBeVisible()
  await page.getByLabel('Tipo de contenido').selectOption('video');await page.getByLabel('Duración').fill('6');await expect(page.getByLabel('Duración')).toHaveValue('6')
  checks.push('Escena guiada abre con Caballito FIRMES preseleccionado, referencia de entorno aislada y fallback manual externo explícito para el E2E sin API.')

  await page.getByRole('button',{name:'Ver instrucción'}).click()
  await page.getByText('Detalles de la instrucción preparada',{exact:true}).click();await expect(page.getByRole('heading',{name:'Instrucción de creación'})).toBeVisible()
  const prompt=page.locator('pre').filter({hasText:'REFERENCE PRIORITY RULES'});await expect(prompt).toContainText('IMAGE 1 = CHARACTER IDENTITY ONLY.');await expect(prompt).toContainText('IMAGE 2 = ENVIRONMENT ONLY.');await expect(prompt).toContainText('Santa Catalina Arch');await expect(prompt).toContainText('shipping containers');await page.getByText('Detalles técnicos',{exact:true}).click();await expect(page.getByText(/REFERENCE A · CHARACTER IDENTITY ONLY/)).toBeVisible();await expect(page.getByText(/REFERENCE B · ENVIRONMENT ONLY/)).toBeVisible()
  ...
  await page.getByRole('button',{name:'Crear producción'}).click();await expect(page).toHaveURL(/\/jobs\//)
  ...
  await page.reload();await expect(page.getByText('TERMINAR EN OTRA HERRAMIENTA')).toBeVisible();await expect(page.getByRole('button',{name:'Copiar instrucción'})).toBeVisible();await expect(page.getByRole('link',{name:/Ver personaje/})).toBeVisible();await expect(page.locator('.external-generation-panel .notice').filter({hasText:'Archivo esperado:'})).toContainText('9:16')
  ...
  await page.reload();await page.getByText('Guía de revisión',{exact:true}).click();await expect(page.getByText('GUÍA DE REVISIÓN',{exact:true})).toBeVisible();await expect(page.getByText('Must be absent: shipping containers')).toBeVisible();checks.push('El resultado vuelve al mismo job y Critic recibe la rúbrica estructurada de la escena.')

  await logout();await login(accounts.critic);await page.goto(origin+'/jobs/'+jobId);await page.getByText('Guía de revisión',{exact:true}).click();await expect(page.getByText('GUÍA DE REVISIÓN',{exact:true})).toBeVisible();await page.getByRole('button',{name:'Apareció vestuario o equipo industrial prohibido.'}).click();await expect(page.getByLabel('Comentarios para la revisión')).toHaveValue('Apareció vestuario o equipo industrial prohibido.');await page.getByRole('button',{name:'Enviar cambio'}).click()
  ...
  await page.reload();await expect(page.getByText('TERMINAR EN OTRA HERRAMIENTA')).toBeVisible();await expect(page.getByRole('link',{name:'Ir a revisión humana'})).toHaveCount(0);checks.push('Un fallo de Critic regenera sobre el mismo job sin bucle blocker→review.')
  ...
  await logout();await login(accounts.critic);await page.goto(origin+'/jobs/'+jobId);await page.getByRole('button',{name:'Aprobar y continuar'}).click();job=await pollJob(jobId,value=>value.status==='WAITING_EXTERNAL_GENERATION'&&value.generation_phase==='VIDEO_FROM_APPROVED_HERO','approved hero did not become image-to-video input');assert.equal(job.id,jobId);assert.equal(job.hero_image.sha256,acceptedSha);assert.equal(job.external_generation_package.generation_phase,'VIDEO_FROM_APPROVED_HERO');const heroRef=job.external_generation_package.references.find(r=>r.role==='APPROVED_HERO_IMAGE');assert.ok(heroRef);assert.equal(heroRef.sha256,acceptedSha);await page.reload();await expect(page.getByText('Ahora anima la imagen aprobada.')).toBeVisible();checks.push('La hero aprobada por Critic queda fijada por SHA como entrada explícita del video.')
  ...
  await logout();await login(accounts.critic);await page.goto(origin+'/jobs/'+jobId);await page.getByRole('button',{name:'Aprobar y continuar'}).click();job=await pollJob(jobId,value=>value.stage==='INDEPENDENT_VERIFIER'&&value.review_state==='AWAITING_REVIEW','final video critic approval did not reach verifier')
  await logout();await login(accounts.verifier);await page.goto(origin+'/jobs/'+jobId);await page.getByRole('button',{name:'Aprobar y continuar'}).click();job=await pollJob(jobId,value=>value.stage==='RELEASE'&&value.review_state==='AWAITING_REVIEW','verifier approval did not reach release')
  await logout();await login(accounts.owner);await page.goto(origin+'/jobs/'+jobId);await page.getByRole('button',{name:'Aprobar entrega'}).click();job=await pollJob(jobId,value=>value.status==='RELEASED','release did not complete');assert.equal(job.artifact_sha256,finalVideoSha)
  const release=await api('/releases/'+job.release_id);assert.equal(release.artifact_sha256,finalVideoSha);assert.equal(release.scene_provenance.result_sha256,finalVideoSha);assert.equal(release.scene_provenance.character_id,'brand_character_firmes_caballito_v1');assert.match(release.scene_provenance.avatar_contract_sha256,/^[a-f0-9]{64}$/);assert.equal(release.scene_provenance.avatar_contract.outfit_id,'avatar_outfit_firmes_base_v1');assert.equal(release.scene_provenance.avatar_contract.brand_marker_policy.mode,'ALWAYS_VISIBLE');assert.equal(release.scene_provenance.approved_hero.sha256,acceptedSha);assert.ok(release.scene_provenance.generation_input_assets.some(value=>value.role==='APPROVED_HERO_IMAGE'&&value.sha256===acceptedSha));assert.equal(release.scene_provenance.generation_prompt_sha256,release.scene_provenance.final_prompt_sha256);assert.equal(release.publication_state,'READY_FOR_MANUAL_PUBLISH')
  await page.reload();await expect(page.getByText('Entrega lista. La publicación en redes sociales se realiza por separado.')).toBeVisible();await page.screenshot({path:join(temp,'scene-released.png'),fullPage:true});checks.push('Critic, verifier y release conservan el hash del video final; la hero aprobada permanece como input trazable y no hay publicación automática.')
