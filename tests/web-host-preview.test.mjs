...
test('existing host login, forward auth, exact workspace probe, logout and revocation share product session',async()=>{
 ...
 try{await writeFile(identityFile,JSON.stringify({users:[{id:'test',username:'operator',password:{salt,hash:scryptSync(password,Buffer.from(salt,'hex'),64).toString('hex')},system_admin:true,memberships:[]},{id:'ordinary',username:'ordinary',password:{salt,hash:scryptSync(password,Buffer.from(salt,'hex'),64).toString('hex')},memberships:[]}]}));const service=createService({dataRoot:join(root,'data'),identityFile,publicOrigin:origin});let cookie=''
 ...
 assert.equal((await send('login',{username:'ordinary',password,locale:'es',next:'/es'})).status,403)
