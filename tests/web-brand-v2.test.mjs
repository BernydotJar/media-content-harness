...
test('live legacy caballito tenant activates FIRMES globally and ships the authorized horse asset',async()=>{
 ...
 assert.match(brand,/LEGACY_FIRMES_TENANTS=new Set\(\['caballito'\]\)/);assert.match(studio,/tenantList\.length===1/);assert.match(workspace,/firmes-hero-mascot/);assert.match(profiles,/El caballito ya forma parte del estudio/);const p=JSON.parse(provenance);assert.equal(p.asset_id,'firmes-caballito-manual-p42');assert.equal(p.source_pdf_page,42);assert.equal(p.sha256,'b17c55ffb0eec46bfa719128ee68a9f18475c45afb90d6d73ce9d82567938397')
})
