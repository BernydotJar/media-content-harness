...
test('controlled single-operator preview lets IT/admin act at Critic and projects the autonomy policy explicitly',async t=>{
 const f=await approvalFixture(t,'controlled_single_operator_preview'),candidate='b'.repeat(64);await f.service.repository.transact(s=>{s.jobs.job={id:'job',tenant_id:f.tenant,status:'AWAITING_REVIEW',stage:'CRITIC',review_state:'AWAITING_REVIEW',candidate_sha:candidate,created_by:'it',graph:{nodes:[]},artifacts:[],evidence:[],approvals:[],blockers:[]};return null});const projected=await f.service.job(f.tokens.it,'job');assert.deepEqual(projected.review_policy,{deployment_class:'controlled_single_operator_preview',mode:'SINGLE_OPERATOR_PREVIEW',single_operator_allowed:true,independence_required:false});await f.service.jobAction(f.tokens.it,'job','approve',{candidate_sha:candidate,review_stage:'CRITIC'});const reviewerView=await f.service.job(f.tokens.coord,'job');assert.equal(reviewerView.review_policy.mode,'INDEPENDENT_REVIEW');assert.equal(reviewerView.review_policy.single_operator_allowed,false);assert.equal(reviewerView.review_policy.independence_required,true)
})

test('V10 interface keeps Google primary, local break-glass, simple team roles and stage-specific review copy',async()=>{const login=await readFile('components/StudioEntry.tsx','utf8'),team=await readFile('components/Team.tsx','utf8'),jobs=await readFile('components/jobs.tsx','utf8'),nav=await readFile('components/studio.tsx','utf8'),page=await readFile('app/[[...segments]]/page.tsx','utf8');assert.match(page,/getService\(\)\.googleAuthConfig\(\)/);assert.match(page,/googleAvailable=\(await getService\(\)\.googleAuthConfig\(\)\)\.available===true/);assert.doesNotMatch(login,/api\('\/auth\/google\/config'/);assert.match(login,/Continuar con Google/);assert.match(login,/Acceso administrativo de contingencia/);assert.match(login,/api\/v1\/auth\/google\/start/);assert.match(team,/Coordinador municipal/);assert.match(team,/IT \/ Aprobación técnica/);assert.match(team,/value=\"editor\"/);assert.match(team,/value=\"viewer\"/);assert.match(team,/Invita por correo/);assert.match(team,/No se envió un correo/);assert.match(team,/Enlace para compartir/);assert.match(team,/Copiar enlace/);assert.match(nav,/label: 'Equipo'/);assert.match(jobs,/TU DECISIÓN/);assert.match(jobs,/¿Esta versión está bien\?/);assert.match(jobs,/Quién puede aprobar esta etapa/);assert.match(jobs,/Coordinación municipal o propietario/);assert.match(jobs,/IT o propietario/);assert.match(jobs,/Propietario o IT/);assert.doesNotMatch(jobs,/Modo operador único activo\./)})
