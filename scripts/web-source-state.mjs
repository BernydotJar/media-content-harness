...
export async function webSourceState(){
 ...
 const files=[...new Set(git('ls-files','--cached','--others','--exclude-standard').split('\n').filter(p=>/^(app|components|assets|server|plugins|schemas|config)\//.test(p)||/^(package\.json|pnpm-lock\.yaml|pnpm-workspace\.yaml|next\.config\.mjs|tsconfig\.json|instrumentation\.ts|scripts\/(build-web|web-source-state|prepare-runtime)\.mjs)$/.test(p)))].sort()
